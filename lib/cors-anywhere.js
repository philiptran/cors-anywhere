...
function proxyRequest(req, res, proxy) {
  // Explicitly reset the request's headers - let the proxy handle it
  req.headers = {};
...
function getHandler(options, proxy) {
  ...
  var hasRequiredHeaders = function(headers) {
    // Make sure request has all required headers, not just some
    // console.log(headers);
    var result = true;
    if (corsAnywhere.requireHeader) {
      for (var i = 0; i < corsAnywhere.requireHeader.length; i++) {
        var headerName = corsAnywhere.requireHeader[i];
  ...
  var jwtCheck = function(req, res, callback) {
    jwt({
      secret: jwks.expressJwtSecret({
        cache: true,
        rateLimit: true,
        jwksRequestsPerMinute: 5,
        jwksUri: corsAnywhere.jwtConfig.jwksUri,
      }),
      audience: corsAnywhere.jwtConfig.audience,
      issuer: corsAnywhere.jwtConfig.issuer,
      algorithms: ['RS256'],
  ...
  var processRequest = (req, res) => {
    ...
    var location = parseURL(req.url.substring(req.url.indexOf('http')));
    ...
    if (corsAnywhere.targetWhitelist.length && corsAnywhere.targetWhitelist.indexOf(targetDomain) === -1) {
      res.writeHead(403, 'Forbidden', cors_headers);
      console.log('The target "' + targetDomain + '" was not whitelisted.');
      res.end('Target is not allowed.');
      return;
    }
    ...
    if (!hasRequiredHeaders(req.headers)) {
      ...
      res.end('Missing required request headers: ' + corsAnywhere.requireHeader);
      return;
    }
    ...
    // console.log('isRequestedOverHttps: ', isRequestedOverHttps);
  ...
  return function(req, res) {
    if (corsAnywhere.jwtConfig.jwtCheck) {
      // Validate JWT Token
